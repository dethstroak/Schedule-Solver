...
public class TimeManager {
	public static LocalTime standardToMilitary(String time) {
		...
		if(time.endsWith("PM")) {	//If afternoon
			...
			}	//Take the format of XX:XXPM			Take the first two numbers and add 12 to it		Add the Colon		Add the minutes
			militaryTime = LocalTime.parse( (Integer.parseInt((time.substring(0, time.indexOf(':')))) + 12) + ":" + time.substring(time.indexOf(':')+1, time.indexOf("P")) );
